...
import {getImportObject, getInstance} from './utils.js';
import {NotImplemented, ProcExit} from './errors.js';
import {ESUCCESS, RAF_PROC_EXIT_CODE} from 'lib/clang/constants.js';

export class App {
  constructor(module, memfs, name, ...args) {
    this.argv = [name, ...args];
    this.environ = {USER: 'arctic'};
    this.memfs = memfs;
  ...
  async run() {
    ...
    try {
      ...
      // Write error message.
      let msg = `\r\n\x1b[91mError: ${exn.message}`;
      if (writeStack) msg = msg + `\r\n${exn.stack}`;
      msg += '\x1b[0m\r\n';
      this.memfs.emit('output', msg);
